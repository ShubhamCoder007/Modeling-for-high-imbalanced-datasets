def get_sample(pv):
    ...
    while len(data_train) <= to_compute:
        ...
        if len(taken1) == 0:
            data_train = np.append(data_train, dt_1.iloc[i:i+1, :].values)
            taken1.append(i)
        if len(taken0) == 0:
            data_train = np.append(data_train, dt_0.iloc[j:j+1, :].values)
            taken0.append(j)
        ...
        if bootstrap == True:
            if inclusion_ratio >= p:
                if i+20 < tot1:
                    data_train = np.append(data_train, dt_1.iloc[i:20+i, :].values)
            ...
            if inclusion_ratio <= p:
                if j+20 < tot0:
                    data_train = np.append(data_train, dt_0.iloc[j:20+j, :].values)
    ...
    data_train = data_train.reshape(int(len(data_train)/numcols), numcols)
    data_train = pd.DataFrame(data_train, columns=colnames)
    return data_train
